...
class ProductionService {
  ...
  async crear(datos) {
    const { produccion_kg, total, devolucion, id_cliente, fecha } = datos;
    if (!id_cliente) {
      throw new Error('El cliente es requerido');
    }
    if (produccion_kg !== undefined && produccion_kg !== null && produccion_kg < 0) {
      throw new Error('La producción en kg no puede ser negativa');
    }
    if (total !== undefined && total !== null && total < 0) {
      throw new Error('El total no puede ser negativo');
    }
    if (devolucion !== undefined && devolucion !== null && devolucion < 0) {
